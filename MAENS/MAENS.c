...
//        indi_copy(&pop[tmp_popsize], &init_indi);
    ...
    // char path_trend[30];
    // strcpy(path_trend, "trend/");
    // strcat(path_trend, map);
    ...
    //     printf("can't open %s \n", path_trend);
    // }
    // fprintf(fp, "MAENS: \n0, %d\t", best_fsb_solution.TotalCost);
    // fclose(fp);
    ...
    while (ite < M_ite && duration < terminal_duration)
    {
        ...
        while (ptr < totalsize)
        {
            ...
            // if(xed_child.Sequence[0] < req_edge_num)
            // {
            //     printf("error\n");
            // }
            // if (xed_child.Loads[0]!=check_load_num(xed_child.Sequence))
            // {
            //     printf("error;\n");
            // }
...
//                printf("Local search %d \n", inst_tasks[1].inverse);
                ...
                // if (mted_child.Loads[0]!=check_load_num(mted_child.Sequence))
                // {
                //     printf("error;\n");
                // }
            ...
            if (duration - duration1 > 0.1)
            {
                // fprintf(fp, "%f, %d\t", duration, best_fsb_solution.TotalCost);
                duration1 = duration;
            }
            if (duration > terminal_duration)
            {
                goto end;
            }
