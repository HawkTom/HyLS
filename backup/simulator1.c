...
void random_stop_for_solution(Individual *Solution, const Task *inst_tasks_vt);
...
void nextScenario(Individual *Solution, Task *inst_tasks_vt, Task *inst_tasks, Arc *inst_arcs, Vehicles *state, unsigned int seed)
{
    ...
    srand(seed);
    random_stop_for_solution(Solution, inst_tasks_vt);
    // printf("seed: %d \n", seed);
...
void random_stop_for_solution(Individual *Solution, const Task *inst_tasks, Rmstate *rmstate)
{
    ...
    {
        int tmp_tour[500], start, end;
        memset(tmp_tour, 0, sizeof(tmp_tour));
        int stop_task = rand_choose(schedule[i][0]) + 1;

        if (stop_task > schedule[i][0])
        {
            continue;
        }

        end = inst_tasks[schedule[i][stop_task]].head_node;
        start = inst_tasks[schedule[i][stop_task-1]].tail_node;
        AssignArray(shortest_path[start][end], tmp_tour);

        RemainRoute[0][0] ++;
        AssignSubArray(schedule[i], stop_task, schedule[i][0], RemainRoute[RemainRoute[0][0]]);
        stop_point[0]++;
        stop_point[stop_point[0]] = tmp_tour[rand_choose(tmp_tour[0])];

        int load = 0;
        for (j = 1; j < stop_task; j++)
        {
            if(schedule[i][j] == 0)
            {
                load = 0;
        ...
        remain_capacity[0]++;
        remain_capacity[remain_capacity[0]] = capacity - load;
    }
