...
//                            printf("single insertion \n");
    ...
    new_step:
        if (improve)
        {
            ...
            update_global_best_solution(&curr_solution, &best_si_solution, inst_tasks, type);
            break;
        } else {
            break;
        }
...
//                                    printf("double insertion \n");
    ...
    new_step:
        ...
        {
            // update to the server
            update_global_best_solution(&curr_solution, &best_di_solution, inst_tasks, type);
            // continue;
            break;
        } else {
            break;
...
int client_swap(Individual *indi, const Task *inst_tasks)
{
    ...
    next_solution = curr_solution;
    // printf("%d %d \n", indi->Sequence[0], best_swap_solution.Sequence[0]);
    while (1)
    {
        next_solution = curr_solution;
        for (trip_u = 1; trip_u <= curr_solution.Route[0][0]; trip_u ++)
...
//                                printf("swap \n");
    ...
    new_step:
        ...
        {
            // update to the server
            update_global_best_solution(&curr_solution, &best_swap_solution, inst_tasks, type);
            // printf("xxx \n");
            break;
        } else {
            break;
    ...
    if ( best_swap_solution.TotalCost != get_task_seq_total_cost(best_swap_solution.Sequence, inst_tasks))
    {
        printf("347: lmsals total cost error \n");
        update_global_best_solution(&curr_solution, &best_swap_solution, inst_tasks, type);
        // longjmp(buf, 2);
        exit(0);
    }
...
int client_reverse(Individual *indi, const Task *inst_tasks)
{
    ...
    new_step:
        ...
        {
            // update to the server
            update_global_best_solution(&curr_solution, &best_rev_solution, inst_tasks, type);
            // improve = 0;
            // continue;
            break;
        } else {
...
int client_cross(Individual *indi, const Task *inst_tasks)
{
    ...
    new_step:
        ...
        {
            // update to the server
            update_global_best_solution(&curr_solution, &best_cross_solution, inst_tasks, type);
            break;
        } else {
            break;
        }
...
void lma_flip(lns_route *curr_solution, lns_route *next_solution, int u, int v, int trip_u, int trip_v, int pos_u, int pos_v, const Task *inst_tasks)
{
    // to be added
}