...
void MASDC(Individual *best_individual, Task *inst_tasks, const int *stop, const int *remain_capacity)
{
    ...
    // while (iter < max_iter && duration < terminal_duration || stop_iter <= terminal_condition)
    while (duration < 2*terminal_duration && stop_iter <= terminal_condition)
    {
        ...
        // printf("duration: %f %d\n", duration, best_individual->TotalCost);
        if (duration > terminal_duration)
        {
            break;
            // fprintf(fp, "%f, %d\t", duration, best_individual->TotalCost);
            // duration1 = duration;
        }
    }
    // fprintf(fp, "\n");
